...
import com.ictasl.java.projectinventory.Persistence.dao.ProjectFileUploadRepository;
...
public class ProjectFileUploadServiceImpl implements ProjectFileUploadService {
	...
	private final static Logger logger = LoggerFactory.getLogger(ProjectFileUploadService.class);

	@Autowired
	private ProjectFileUploadRepository projectFileUploadRepository;
	...
	public ResponseMessage uploadFiles(MultipartFile[] files,long orgId) throws Exception {
		...
		//List<ProjectFileEntity> projectFileList = projectFileUploadRepository.findAllByOrgIdAndUserId(organizationId,userId);
		List<ProjectFileEntity> projectFileList = projectFileUploadRepository.findAllByOrgId(organizationId);
		List<ProjectFileEntity> fileList = new ArrayList<>();
		...
		for (MultipartFile multipartFile : files) {
			...
			if (!projectFileList.isEmpty()) {
				for (ProjectFileEntity projectFileEntity : projectFileList) {
		...
		if (availableFileList.isEmpty()) {
			for (MultipartFile multipartFile : files) {
				...
				try {
					...
					projectFileUploadRepository.saveAndFlush(entity);
				} catch (IOException e) {
	...
	public List<HashMap<String, Object>> getUserRelatedFiles(HttpServletRequest request,long orgId) throws Exception{
		...
		/*List<ProjectFileEntity> projectFileList = projectFileUploadRepository.findAllByOrgIdAndUserId(organizationId,
				userId);*/
		List<ProjectFileEntity> projectFileList = projectFileUploadRepository.findAllByOrgId(organizationId);
	...
	public List<HashMap<String, Object>> loadAllFiles(HttpServletRequest request)throws Exception {
		List<ProjectFileEntity> projectFileList = projectFileUploadRepository.findAll();
		List<HashMap<String, Object>> returnFileList = makeFileUrl(projectFileList , request);
		return returnFileList;
	}
	
	private List<HashMap<String, Object>> makeFileUrl(List<ProjectFileEntity> projectFileList, HttpServletRequest request) throws Exception{
	...
	public ResponseMessage deleteFile(long id) throws Exception {
		...
		Optional<ProjectFileEntity> projectFileList = projectFileUploadRepository.findById(id);
		...
		if (f.delete()) {
			projectFileUploadRepository.deleteById(id);
	...
	public Resource downloadFile(long id) throws Exception {
		Optional<ProjectFileEntity> projectFileEntity = projectFileUploadRepository.findById(id);
